...
function CompilationStatus({ success, timedOut }) {
  ...
  return (
    <span
      ...
      className="ml-2"
    >
      <FontAwesomeIcon className={`text-${success?'green':'red'}-500`} icon={['far', success?'check':'times']}/>
    </span>
  )
} 

export function CompileStageReportView({ reports }) {
    ...
    <div className="bg-white shadow rounded-lg border">
      <header className="bg-gray-100 w-full px-4 py-3 rounded-tl-lg rounded-tr-lg border-b flex justify-between items-center">
        <div>
          ...
          <h6 className="text-xs text-gray-600">Pipeline stage for program compilation</h6>
        </div>
        <span
      ...
      <div className="px-4 py-3 flex flex-col justify-between">
        ...
        <div className="my-4 rounded-md bg-gray-700 border p-3 text-white font-mono h-32 overflow-y-auto">
          ...
          {
            reports[viewingIndex][logView].length===0 && (<pre className="text-center">No logged output</pre>) 
          }
        </div>
      </div>
    </div>
